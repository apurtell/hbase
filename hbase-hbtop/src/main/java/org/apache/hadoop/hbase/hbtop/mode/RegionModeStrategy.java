...
public final class RegionModeStrategy implements ModeStrategy {
  ...
  public List<Record> getRecords(ClusterStatus clusterStatus) {
    ...
    for (ServerName sn: clusterStatus.getServers()) {
      ...
      long lastReportTimestamp = sl.getReportTime();
      for (RegionLoad rl: sl.getRegionsLoad().values()) {
        ret.add(createRecord(sn, sl, rl, lastReportTimestamp));
      }
    }
    return ret;
  }

  private Record createRecord(ServerName sn, ServerLoad serverMetrics, RegionLoad regionMetrics,
    ...
    String replicaId = "";
    try {
      byte[][] elements = HRegionInfo.parseRegionName(regionMetrics.getName());
      ...
      region = HRegionInfo.encodeRegionName(regionMetrics.getName());
    ...
    requestCountPerSecond.refresh(lastReportTimestamp, regionMetrics.getReadRequestsCount(),
      regionMetrics.getWriteRequestsCount());
    ...
    builder.put(Field.STORE_FILE_SIZE, new Size(regionMetrics.getStorefileSizeMB(), Unit.MEGABYTE));
    builder.put(Field.UNCOMPRESSED_STORE_FILE_SIZE,
      new Size(regionMetrics.getStoreUncompressedSizeMB(), Unit.MEGABYTE));
    builder.put(Field.NUM_STORE_FILES, regionMetrics.getStorefiles());
    builder.put(Field.MEM_STORE_SIZE, new Size(regionMetrics.getMemStoreSizeMB(), Unit.MEGABYTE));
    builder.put(Field.LOCALITY, regionMetrics.getDataLocality());

    long compactingCellCount = regionMetrics.getTotalCompactingKVs();
    long compactedCellCount = regionMetrics.getCurrentCompactedKVs();
    ...
    long lastMajorCompactionTimestamp = regionMetrics.getLastMajorCompactionTs();
