...
public final class TestUtils {
  ...
  public static ClusterStatus createDummyClusterMetrics() {
    ...
    serverLoads.put(host1,
      createServerMetrics(100,
        new Size(100, Size.Unit.MEGABYTE), new Size(200, Size.Unit.MEGABYTE), 100,
        Lists.newArrayList(
          createRegionMetrics("table1,,1.00000000000000000000000000000000.", 100, 100,
          ...
          createRegionMetrics("table2,1,2.00000000000000000000000000000001.", 200, 200,
    ...
    serverLoads.put(host2,
      createServerMetrics(200,
        new Size(16, Size.Unit.GIGABYTE), new Size(32, Size.Unit.GIGABYTE), 200,
        Lists.newArrayList(
          createRegionMetrics("table1,1,4.00000000000000000000000000000003.", 100, 100,
            new Size(100, Size.Unit.MEGABYTE), new Size(200, Size.Unit.MEGABYTE), 1,
            new Size(100, Size.Unit.MEGABYTE), 0.4f, 50, 100, "2019-07-22 00:00:03"),
          createRegionMetrics("table2,,5.00000000000000000000000000000004.", 200, 200,
            new Size(200, Size.Unit.MEGABYTE), new Size(400, Size.Unit.MEGABYTE), 2,
            new Size(200, Size.Unit.MEGABYTE), 0.5f, 150, 200, "2019-07-22 00:00:04"),
          createRegionMetrics("namespace:table3,,6.00000000000000000000000000000005.", 300, 300,
            new Size(300, Size.Unit.MEGABYTE), new Size(600, Size.Unit.MEGABYTE), 3,
  ...
  private static ClusterStatusProtos.RegionLoad createRegionMetrics(String regionName,
  ...
  private static ServerLoad createServerMetrics(long reportTimestamp,
    Size usedHeapSize, Size maxHeapSize, long requestCountPerSecond,
    List<ClusterStatusProtos.RegionLoad> regionMetricsList) {
    return new ServerLoad(ClusterStatusProtos.ServerLoad.newBuilder()
        ...
        .addAllRegionLoads(regionMetricsList)
        .build());
  }
  ...
  private static void assertRecordInRegionMode(Record record, String namespace, String startCode,
    ...
    assertEquals(record.size(), 21);
    assertEquals(record.get(Field.NAMESPACE).asString(), namespace);
    assertEquals(record.get(Field.START_CODE).asString(), startCode);
    assertEquals(record.get(Field.REPLICA_ID).asString(), replicaId);
    assertEquals(record.get(Field.TABLE).asString(), table);
    assertEquals(record.get(Field.REGION).asString(), region);
    assertEquals(record.get(Field.REGION_SERVER).asString(), regionServer);
    assertEquals(record.get(Field.LONG_REGION_SERVER).asString(), longRegionServer);
    assertEquals(record.get(Field.REQUEST_COUNT_PER_SECOND).asLong(),
      requestCountPerSecond);
    assertEquals(record.get(Field.READ_REQUEST_COUNT_PER_SECOND).asLong(),
      readRequestCountPerSecond);
    assertEquals(record.get(Field.WRITE_REQUEST_COUNT_PER_SECOND).asLong(),
      writeCountRequestPerSecond);
    assertEquals(record.get(Field.STORE_FILE_SIZE).asSize(), storeFileSize);
    assertEquals(record.get(Field.UNCOMPRESSED_STORE_FILE_SIZE).asSize(),
      uncompressedStoreFileSize);
    assertEquals(record.get(Field.NUM_STORE_FILES).asInt(), numStoreFiles);
    assertEquals(record.get(Field.MEM_STORE_SIZE).asSize(), memStoreSize);
    assertEquals(record.get(Field.LOCALITY).asFloat(), Locality, 0.001);
    assertEquals(record.get(Field.START_KEY).asString(), startKey);
    assertEquals(record.get(Field.COMPACTING_CELL_COUNT).asLong(), compactingCellCount);
    assertEquals(record.get(Field.COMPACTED_CELL_COUNT).asLong(), compactedCellCount);
    assertEquals(record.get(Field.COMPACTION_PROGRESS).asFloat(), compactionProgress, 0.001);
    assertEquals(record.get(Field.LAST_MAJOR_COMPACTION_TIME).asString(),
  ...
  private static void assertRecordInNamespaceMode(Record record, long requestCountPerSecond,
    ...
    assertEquals(record.get(Field.NUM_STORE_FILES).asInt(), numStoreFiles);
    assertEquals(record.get(Field.MEM_STORE_SIZE).asSize(), memStoreSize);
    assertEquals(record.get(Field.REGION_COUNT).asInt(), regionCount);
  }
  ...
  private static void assertRecordInRegionServerMode(Record record, String longRegionServer,
    ...
    assertEquals(record.get(Field.LONG_REGION_SERVER).asString(),
      longRegionServer);
    assertEquals(record.get(Field.REQUEST_COUNT_PER_SECOND).asLong(),
      requestCountPerSecond);
    assertEquals(record.get(Field.READ_REQUEST_COUNT_PER_SECOND).asLong(),
    ...
    assertEquals(record.get(Field.MEM_STORE_SIZE).asSize(), memStoreSize);
    assertEquals(record.get(Field.REGION_COUNT).asInt(), regionCount);
    assertEquals(record.get(Field.USED_HEAP_SIZE).asSize(), usedHeapSize);
    assertEquals(record.get(Field.MAX_HEAP_SIZE).asSize(), maxHeapSize);
  }

  public static void assertSummary(Summary summary) {
    assertEquals(summary.getVersion(), HBASE_VERSION);
    assertEquals(summary.getClusterId(), CLUSTER_UUID);
    assertEquals(summary.getServers(), 3);
    assertEquals(summary.getLiveServers(), 2);
    assertEquals(summary.getDeadServers(), 1);
    assertEquals(summary.getRegionCount(), 6);
    assertEquals(summary.getRitCount(), 1);
    assertEquals(summary.getAverageLoad(), 3.0, 0.001);
    assertEquals(summary.getAggregateRequestPerSecond(), 300L);
  }
}
