...
public class IntegrationTestCompactionWithDeviceSimulator extends IntegrationTestBase {
  ...
  static {
    Map<String, Class<? extends BlockCompressedSizePredicator>> m = new HashMap<>();
    m.put("PreviousBlockCompressionRatePredicator", PreviousBlockCompressionRatePredicator.class);
    m.put("UncompressedBlockSizePredicator", UncompressedBlockSizePredicator.class);
    PREDICATOR_CLASSES = java.util.Collections.unmodifiableMap(m);
  }
  ...
  private long loadData(HBaseTestingUtility testUtil, long totalDataBytes, int valueSize,
  ...
  private HStore getStore(HBaseTestingUtility testUtil) {
  ...
  private void waitForFlush(HBaseTestingUtility testUtil) throws IOException {
