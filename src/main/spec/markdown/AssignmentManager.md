...
**CreateTable forward path** (no feature toggle; no WF on `CreateTablePrepare`)

```tla
  \* -- CreateTable forward path --
  \/ \E t \in Tables: \E r \in Regions: create!CreateTablePrepare(t, r)
  \/ \E t \in Tables: create!CreateTableDone(t)
```
...
**CreateTable completion** (deterministic; no WF on `CreateTablePrepare`)

```tla
  \* CreateTable completion (deterministic; no WF on CreateTablePrepare)
  /\ \A t \in Tables: WF_vars(create!CreateTableDone(t))
```
