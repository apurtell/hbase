...
Minimal ZooKeeper model for the HBase AssignmentManager. Models ZK ephemeral nodes for RegionServer liveness. Each live RS holds an ephemeral node; when the RS process dies, ZK detects the session expiry and deletes the node. The master reads these nodes to determine which servers are alive.

ZK is the ground truth about RS liveness, independent of master state. An RS can die at any time and ZK will detect it regardless of whether the master is alive or not.

Contains one action: `ZKSessionExpire(s)`.
...
The RS process may still be a zombie briefly after ZK session expiry (`rsOnlineRegions` is NOT cleared here — `RSAbort` handles that). This preserves the zombie window that makes WAL fencing necessary for correctness.
