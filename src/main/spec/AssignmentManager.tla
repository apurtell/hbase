 ...
 *   - Disable:        DisableTableProcedure (table disable, region close)
 *   - Enable:         EnableTableProcedure (table enable, region open)
 *   - Modify:         ModifyTableProcedure (table alter, rolling region reopen)
 *
...
\* tableEnabled[t] \in TableStateSet — tracks the table lifecycle state.
\* Values: "ENABLED" (default), "DISABLING" (disable in progress),
\*         "DISABLED" (disabled), "ENABLING" (enable in progress).
\* DisableTablePrepare sets tableEnabled[t] = "DISABLING";
\* DisableTableDone sets tableEnabled[t] = "DISABLED";
\* EnableTablePrepare sets tableEnabled[t] = "ENABLING";
\* EnableTableDone sets tableEnabled[t] = "ENABLED".
\*
...
\* SPLIT, SPLITTING_NEW, MERGED, or MERGING_NEW must NOT have a location.
    ...
    \A r \in Regions:
      RegionExists(r) =>
        ...
        \/ /\ regionState[r].state = "SPLITTING_NEW"
           /\ metaTable[r].state = "CLOSED"
        \* MERGING_NEW merged region: in-memory is MERGING_NEW
        \* while meta is CLOSED (same pattern as SPLITTING_NEW).
          ...
          ( regionState[r].procType # "NONE" =>
              \/ /\ regionState[r].procType = "ASSIGN"
                 /\ regionState[r].state \in
                      ...
                      \* MERGING_NEW: merged region enters ASSIGN via
                      \* MergeUpdateMeta (same pattern as SPLITTING_NEW).
                      }
              ...
              \/ /\ regionState[r].procType = "MERGE"
                 /\ regionState[r].state \in
                      { "CLOSED",
                        "MERGED",
                        "MERGING_NEW"
                      \* CLOSED: target after close, before PONR
                      \*   (MergeCheckClosed re-attaches MERGE lock).
                      \* MERGED: target after PONR.
                      \* MERGING_NEW: merged region lock before open yield.
                      }
          )
    )
          ...
          ( regionState[r].procType \in
              ...
              /\ ( regionState[r].procStep = "GET_ASSIGN_CANDIDATE" =>
                     regionState[r].state \in
                       ...
                       \* MERGING_NEW: merged region in ASSIGN at
                       \* GET_ASSIGN_CANDIDATE (MergeUpdateMeta).
                       }
                 )
              ...
              /\ ( regionState[r].procStep = "CLOSE" =>
                 ...
                 \* MERGING: merge-yielded UNASSIGN starts from MERGING.
                 )
...
SplitMergeMutualExclusion ==
  masterAlive = TRUE =>
    \A r \in Regions:
      \* A daughter/merged region cannot also be tracked as a parent procedure target.
      /\ ( regionState[r].state = "SPLITTING_NEW" =>
             parentProc[r] = NoParentProc
         )
      /\ ( regionState[r].state = "MERGING_NEW" => parentProc[r] = NoParentProc
         )
...
\* NoOrphanedMergedRegion: a region in MERGING_NEW state always has
\* an ASSIGN procedure (child TRSP from MergeUpdateMeta).
...
\* region is only materialized at PONR (MergeUpdateMeta).
MergeAtomicity ==
  masterAlive = TRUE =>
    \A r \in Regions:
...
\* of that table must also have parentProc.type = "TRUNCATE".  The truncate
\* procedure locks the entire table atomically in TruncatePrepare; no region
\* of the table can escape the lock.
\*
\* Source: TruncateTableProcedure acquires exclusive table lock for all
\*         regions before any state mutation.
TruncateAtomicity ==
  masterAlive = TRUE =>
    \A t \in Tables:
...
\* ModifyTableSafety: While a MODIFY is in progress on table t, every
\* region with parentProc.type = "MODIFY" is either:
\*   (a) OPEN with no procedure (completed reopen or awaiting ModifyTableDone),
\*   (b) in a valid TRSP REOPEN state (procType = "REOPEN"),
\*   (c) handled by SCP (ABNORMALLY_CLOSED with ASSIGN),
\*   (d) skipped (parentProc.step = "COMPLETING"), or
\*   (e) in a valid TRSP state from another procedure (mid-split/merge/SCP).
\* Ensures the rolling restart does not lose regions.
\*
\* Source: ReopenTableRegionsProcedure skips busy regions;
\*         TRSP REOPEN drives the close/open cycle;
\*         SCP handles server crashes during reopen.
ModifyTableSafety ==
  masterAlive = TRUE =>
    \A r \in Regions:
...
Next ==
  ...
  \* -- Merge forward path + rollback (gated on UseMerge) --
  \/ ( UseMerge /\ \E r1, r2, m \in Regions: merge!MergePrepare(r1, r2, m) )
  \/ ( UseMerge /\ \E r1 \in Regions: merge!MergeCheckClosed(r1) )
...
Fairness ==
  ...
  \* No fairness on RSCloseNotFound (fires non-deterministically; conditional).
  \* SCP state machine
  ...
  \* Merge forward path (deterministic steps; no WF on MergePrepare/MergeFail)
...
\* Liveness: once an ASSIGN procedure is attached to an OFFLINE region,
\* the region eventually reaches OPEN.  This tests the full TRSP assign
\* pipeline under fairness (candidate selection -> dispatch -> RS open ->
\* report -> persist).
\*
\* Precondition: procedure must be ASSIGN (not merely OFFLINE with no
\* procedure, since TRSPCreate has no fairness -- the decision to create
\* an ASSIGN is an external event).
\*
...
\* deterministic, so forward progress is guaranteed.
\*
...
\* Liveness: regions in transitional states (OPENING, CLOSING)
\* eventually leave those states.  A stuck region would indicate
\* a lost procedure, missing fairness, or RS liveness failure.
\*
\* Gated on RegionExists: non-existent regions (NoRange) are
\* excluded from the liveness check.
\*
...
\* Safety: MODIFY regions are in valid states during rolling restart.
