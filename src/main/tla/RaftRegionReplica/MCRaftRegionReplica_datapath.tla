 ...
 * CatchUpDataIntegrity, FollowerFlushMemstoreDrop, etc.) depend on
 ...
 * ----------------------------------
 * All 20 invariants are checked.  Coverage status:
 *
 *   Non-trivial (primary verification in this domain):
 *     RaftLogConsistency, CatchUpDataIntegrity, FollowerSeqIdConsistency,
 *     FlushAtomicity, NoOrphanMemstoreDrop, FlushWriteExclusion,
 *     FollowerFlushMemstoreDrop, HFilesBeforeFlushMarker,
 *     NoFlushDuplication, PromotionReadWriteGuard,
 *     PromotionMVCCContinuity, CatchUpCompleteness
 *
 *   Non-trivial (verified here and in election domain):
 *     LeaderUniqueness, LeaseImpliesLeadership, LeaseExpiresBeforeElection
 *     (zero-drift case only), PartitionSymmetric
 *
 *   Vacuous in this domain (primary verification in election domain):
 *     ClockDriftBounded      -- trivially true because MaxClockDrift = 0
 ...
 *                               raftCommitted = TRUE)
 *     WALSyncFailureSafety   -- trivially true because WALSyncFail is
 *                               removed; walSync never reaches "Failed"
 *     MarkerBreaksBatch      -- trivially true because fApplyBatch is
 *                               merged away (always {}); batch boundary
 *                               enforcement is still correct by
 *                               construction in FollowerBatchApply
 *
 * COMPLEMENTARY DOMAIN
 * --------------------
 * The election domain (MCRaftRegionReplica_election) uses the original
 * unmodified Next with all 31 actions, full timing (MaxClockDrift = 1,
 * ElectionTimeoutMin = 4), and MaxSeqId = 1.  It provides primary
 * verification for the 4 invariants that are vacuous here.  Simulation
 * mode (MCRaftRegionReplica_sim) exercises both domains simultaneously
 * with MaxSeqId = 5 and full timing, providing statistical coverage of
 * the cross-product that neither exhaustive domain covers alone.
 *)
