 ...
 * with all 31 actions — no action merges, no action removals.  The only
 ...
 * ----------------------------------
 * All 19 invariants are checked with the full unmodified spec.
 * Coverage status:
 *
 *   Primary verification (this is the only domain that exercises these):
 *     ClockDriftBounded        -- MaxClockDrift = 1 enables drift states
 *     LeaseExpiresBeforeElection -- drift-induced lease overlap tested
 *     WriteBarrierSafety       -- full Applied writePhase (no action merge)
 *     WALSyncFailureSafety     -- WALSyncFail + WALFailureAbort present
 *     MarkerBreaksBatch        -- two-step batch apply (fApplyBatch
 *                                 transitions between {} and non-{})
 *
 *   Non-trivial (verified here and in datapath domain):
 *     LeaderUniqueness, LeaseImpliesLeadership, RaftLogConsistency,
 *     CatchUpDataIntegrity, PartitionSymmetric, FollowerSeqIdConsistency,
 *     FlushAtomicity, NoOrphanMemstoreDrop, FollowerFlushMemstoreDrop,
 ...
 *     NoFlushDuplication     -- requires orphan flush + re-flush (needs
 *                               MaxSeqId >= 2); trivially satisfied here
 ...
 * shallow coverage here (FlushWriteExclusion, NoFlushDuplication).
 *
