...
**MicroRaft core.** The engine imports MicroRaft's single-threaded actor RAFT core (Apache 2.0) into the HBase source tree under the HBase namespace. The per-node executor model is replaced with a shared multi-group actor pool with two-lane mailboxes. The per-node `RaftStore` is replaced by a sharded multiplexed unified log. The per-node `Transport` is replaced by a process-wide coalescing transport with cross-group `BatchAppendEntries` and a store-level heartbeat sweep. Finally, the leader lease is rewritten to be drift-compensated and to refresh on every voting-quorum response. Everything else -- pre-vote, leader stickiness, vote durability before response, immediate post-election heartbeat, new-term entry on election, term fencing, deterministic commit-index advancement, parallel leader flush, single-server membership change with the committed-entry-in-current-term guard, and leadership transfer with stickiness bypass -- is carried over unchanged.
...
- **Tier B (default, page-cache + segment-roll + periodic).** `persistLogEntries`, `persistSnapshotChunk`, `truncateLogEntriesFrom`, `truncateLogEntriesUntil`, and `deleteSnapshotChunks` enqueue `(fsyncRequested=false, mandatoryFsync=false)`. The shard fsyncs (i) when the active segment rolls (`rollSegment` writes a `SEGMENT_FOOTER`, truncates back if pre-allocated, calls `force(true)`) and (ii) on the periodic timer thread (`UnifiedRaftStore.periodicFsyncTick`), which submits a mandatory-fsync barrier every `hbase.consensus.log.fsync.interval.ms` (default 10000 ms; `<=0` disables) whenever `WriterShard.unflushedBytes() > 0`. `RaftStore.flush()` in this mode returns after the gathered write returns -- without an inline fsync. This is the right operating point for embeddings whose data durability lives elsewhere (HBase's HDFS WAL, for example): the consensus log's role is to keep follower memstores warm for fast promotion, not to be the system-level data durability surface.
...
`GroupReplayState.apply` reconstructs a `RestoredRaftState` per group: the latest in-flight snapshot is kept, `TRUNCATE_FROM` and `TRUNCATE_UNTIL` are applied to the in-memory entry list, and entries up to the latest completed `flushedSnapshotEntry` are dropped. `load()` returns `null` for any group missing `localEndpoint` or `initialGroupMembers`, so the engine treats it as a fresh-start group rather than a partially recovered one. Both classes of failure -- corruption truncation and missing-prerequisite drop -- share the same upper-layer recovery path: the RAFT layer above closes the resulting gap via standard `AppendEntries` for small gaps, or `InstallSnapshot` for large gaps. A node with a short log cannot win election (RAFT's up-to-date check), so a would-be leader with corruption automatically defers to a peer with intact state.
...
`onApplyBatchEnd()` calls `drainPending()` which fires `ConsensusSpi.onCommit(...)` with the entire buffered batch and -- if metrics are present -- updates `commitApplyTime`, `commitBatchSize`, and `commitBatchBytes` as a unit. This is the level at which the embedding can amortize per-batch fixed costs (one MVCC bracket, one memstore add, one complete-and-wait for the HBase region embedding) across N entries times M mutations.
...
The engine is intentionally generic. There is no region-aware code anywhere in `hbase-consensus`: `ConsensusServer`'s public surface operates on opaque group identifiers, opaque peer identifiers, opaque `byte[]` log entries, and opaque `byte[]` snapshots. Any region-specific behavior -- mapping HBase regions to RAFT group ids (deterministically, by hashing table name plus start key plus region id and deliberately excluding the replica id so all replicas of a region join the same group), mapping `(server name, replica id)` to peer ids, opening a group on region open, removing it on region close, transferring leadership for graceful rebalancing, plumbing flush sequence ids into `FlushMarker` operations, and reacting to `onLeaderElected` by issuing a master-coordination RPC -- lives in a thin region-replica adapter that is implemented outside this module. The engine documents only the SPI surface that adapter consumes.
...
| TLA+ name | English meaning |
|---|---|
| `BoundedControlLatency` | Every head-arrival control task -- one that finds an empty control mailbox at enqueue -- has `serviceStart - enqueueTime <= L = BulkBatchCap * ServiceCost`. The bound is asymmetric: control-lane head-arrival wait is bounded by the bulk burst cost, not by the control-batch cap. The bound is tight: `L = 1` is provably violated. |
