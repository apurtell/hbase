...
public final class LeadershipTransferState {
  ...
  /**
   * Attaches the given Future object to the current leadership transfer process, if it aims the
  ...
  void andThen(RaftEndpoint targetEndpoint, OrderedFuture otherFuture) {
    if (this.endpoint.equals(targetEndpoint)) {
      ...
      otherFuture.fail(new IllegalStateException(
        "There is an ongoing leadership transfer process to " + endpoint));
    }
  }
}
