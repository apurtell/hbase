...
public interface RaftLogStats {
  ...
  /**
   * Returns the last term in the Raft log, either from the last log entry or from the last locally
   * taken or installed snapshot.
   * @return the last term in the Raft log, either from the last log entry or from the last locally
   *         taken or installed snapshot
   */
  int getLastLogOrSnapshotTerm();

  /**
   * Returns the last log entry index in the Raft log, either from the last log entry or from the
   * last locally taken or installed snapshot.
   * @return the last log entry index in the Raft log, either from the last log entry or from the
   *         last locally taken or installed snapshot
   */
  ...
  /**
   * Returns the indices of the last known appended Raft log entries on the followers. This map is
   * non-empty only for the leader Raft node.
