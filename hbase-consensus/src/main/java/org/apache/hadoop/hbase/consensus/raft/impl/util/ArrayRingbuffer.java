 ...
 * <p>
 * Currently the Ringbuffer is not a partitioned data-structure. So all data of a ringbuffer is
 * stored in a single partition and replicated to the replicas. No thread safety is needed since a
...
public final class ArrayRingbuffer<E> {
  ...
  private void checkReadSequence(long sequence) {
    if (sequence > tailSequence) {
      ...
      throw new IllegalArgumentException(
        "sequence:" + sequence + " is too small. The current headSequence: " + headSequence + " "
