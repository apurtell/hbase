...
public final class Long2ObjectHashMap<V> implements Map<Long, V> {
  ...
  /**
   * Construct a new map allowing a configuration for initial capacity and load factor. for the
   * backing array limit for resizing on puts
   */
  ...
  private static void validateInitialCapacity(int initialCapacity) {
    ...
    if (initialCapacity > MAX_CAPACITY) {
      throw new IllegalArgumentException(
        "Initial capacity cannot exceed " + MAX_CAPACITY + ": " + initialCapacity);
    }
  }
  ...
  /**
   * Get the actual threshold which when reached the map resize. This is a function of the current
   ...
   * {@link LongFunction} and put it in the map. to search on. to provide a value if the get returns
   * null.
  ...
  /**
   * Overloaded version of {@link Map#containsKey(Object)} that takes a primitive long key. for
   * indexing the {@link Map}
  ...
  /**
   * Overloaded version of {@link Map#get(Object)} that takes a primitive long key. for indexing the
   * {@link Map}
  ...
  /**
   * Overloaded version of {@link Map#put(Object, Object)} that takes a primitive long key. for
   * indexing the {@link Map} to be inserted in the {@link Map}
   ...
   * <code>next()</code> method can just return the iterator instance itself with no heap
   * allocation. This characteristic makes the set unusable wherever the returned entries are
