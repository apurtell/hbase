 ...
 */
package org.apache.hadoop.hbase.consensus.raft.report;
 ...
 * consensus algorithm.
 * <p>
 * Raft node reports are published either periodically or when there is a change in the local state
 * of a Raft node related to the Raft consensus algorithm. The duration of the periodic report
 * publishing is configured via {@link RaftConfigBuilder#setRaftNodeReportPublishPeriodSecs(int)}.
...
public interface RaftNodeReport {
   ...
   * from the currently effective applied member list, if there is an ongoing (appended but not-yet
   * committed) membership change in the group. It can be different from the current committed
   * member list of the Raft group, also if a new membership change is committed by other Raft nodes
   * of the group but not learnt by this Raft node yet.
   ...
   * in the last {@link RaftConfig#leaderHeartbeatTimeoutSecs} seconds.
  ...
  /**
   * Returns timestamps of latest heartbeats sent by the non-leader nodes to the leader Raft node,
   * including both {@link RaftRole#FOLLOWER} and {@link RaftRole#LEARNER} nodes. The leader node's
   * RaftEndpoint is not present in the map. This map is returned non-empty only by the leader Raft
   * node.
  ...
  enum RaftNodeReportReason {
    ...
    TAKE_SNAPSHOT,
    /**
     * The report is created when a {@link RaftNode} installs a snapshot that is sent by the current
