...
import org.apache.hadoop.hbase.consensus.raft.impl.util.ArrayRingbuffer;
 ...
 * <p>
 * The entries appended in the Raft group leader's log are replicated to all followers in the same
 * log order and all Raft nodes in a Raft group eventually keep the exact same copy of the Raft log.
 * <p>
 * Raft maintains the following properties, which together constitute the LogMatching Property:
 * <ul>
 * <li>If two entries in different logs have the same index and term, then they store the same
 * command.</li>
 * <li>If two entries in different logs have the same index and term, then the logs are identical in
 * all preceding entries.</li>
 * </ul>
 * @see LogEntry
 * @see SnapshotEntry
 */
public final class RaftLog {
  ...
  /**
   * Returns the log entry stored at {@code entryIndex}. Entry is retrieved only from the current
   * log, not from the snapshot entry.
   * <p>
  ...
  /**
   * Returns true if the log contains an entry at {@code entryIndex}, false otherwise.
   * <p>
   * Important: Log entry indices start from 1, not 0.
   */
   ...
   * @return truncated log entries If no entries are available to truncate, if {@code entryIndex} is
  ...
  public List<LogEntry> truncateEntriesFrom(long entryIndex) {
    if (entryIndex <= snapshotIndex()) {
      throw new IllegalArgumentException(
        "Illegal index: " + entryIndex + ", snapshot index: " + snapshotIndex());
    ...
    assert startSequence >= log.headSequence()
      : "Entry index: " + entryIndex + ", Head Seq: " + log.headSequence();
  ...
  /**
   * Returns the last entry in the Raft log, either from the last log entry or from the last
   * snapshot if no logs are available.
   */
  ...
  /**
   * Appends new entries to the Raft log. If an entry is appended with a lower term than the last
   * term in the log or if an entry is appended with an index not equal to
   * {@code index == lastIndex + 1}. if the Raft log has no enough available capacity
   */
  public void appendEntries(List<LogEntry> entries) {
    ...
    try {
      ...
      throw new RaftException("Could not persist entries from start index: "
  ...
  private void validateForAppend(List<LogEntry> entries) {
    if (!checkAvailableCapacity(entries.size())) {
      throw new IllegalStateException("Not enough capacity! Capacity: " + log.getCapacity()
        + ", Size: " + log.size() + ", New entries:" + " " + entries.size());
    }
  ...
  /**
   * Returns the last term in the Raft log, either from the last log entry or from the last snapshot
   */
  ...
  /**
   * Appends the given entry to the Raft log. If an entry is appended with a lower term than the
   * last term in the log or if an entry is appended with an index not equal to
   * {@code index == lastIndex + 1}. if the Raft log has no available capacity
   */
  public void appendEntry(LogEntry entry) {
    ...
    try {
      store.persistLogEntries(Arrays.asList(entry));
    } catch (IOException e) {
  ...
  /**
   * Returns log entries between {@code fromEntryIndex} and {@code toEntryIndex}, both inclusive. If
   * {@code fromEntryIndex} is greater than {@code toEntryIndex}, or if {@code fromEntryIndex} is
   * equal to / smaller than {@code snapshotIndex}, or if {@code fromEntryIndex} is greater than
   * last log index, or if {@code toEntryIndex} is greater than last log index.
   */
  public List<LogEntry> getLogEntriesBetween(long fromIndexInclusive, long toIndexInclusive) {
    if (fromIndexInclusive > toIndexInclusive) {
      throw new IllegalArgumentException("Illegal from entry index: " + fromIndexInclusive
        + ", to entry index: " + toIndexInclusive);
  ...
  /**
   * Installs the snapshot entry and truncates log entries those are included in snapshot (entries
   * whose indexes are smaller than the snapshot's index). The given snapshot is already persisted
   * and flushed to the storage before calling this method.
   * @return truncated log entries after snapshot is installed if the snapshot's index is smaller
   *         than or equal to the current snapshot index
   */
  ...
  /**
   * Installs the snapshot entry and truncates log entries until the given index which is smaller
   * than or equal to the snapshot index. The given snapshot is already persisted and flushed to the
  ...
  public int setSnapshot(SnapshotEntry snapshot, long truncateUpToIndex) {
    ...
    try {
      ...
      throw new RaftException(
        "Failed to truncate log entries from persistence until index=" + truncateUpToIndex, null,
        e);
    }
  ...
  public static int getLogCapacity(int commitCountToTakeSnapshot, int maxUncommittedLogEntryCount) {
    int maxLogEntryCountToKeepAfterSnapshot =
      Math.max(1, (int) (commitCountToTakeSnapshot * KEPT_LOG_ENTRY_RATIO_BEFORE_SNAPSHOT_INDEX));
    return commitCountToTakeSnapshot + maxUncommittedLogEntryCount
