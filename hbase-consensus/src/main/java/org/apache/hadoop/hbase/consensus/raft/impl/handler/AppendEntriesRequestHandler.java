...
public class AppendEntriesRequestHandler extends AbstractMessageHandler<AppendEntriesRequest> {
  ...
  @SuppressWarnings({ "checkstyle:npathcomplexity", "checkstyle:cyclomaticcomplexity",
    "checkstyle:methodlength", "checkstyle:nestedifdepth" })
  ...
  private Entry<Long, List<LogEntry>> appendLogEntries(AppendEntriesRequest request, RaftLog log) {
    ...
    if (request.getLogEntries().size() > 0) {
      ...
      long lastLogIndex = log.lastLogOrSnapshotIndex();
      for (int i = 0, requestEntryCount = request.getLogEntries().size(); i
          < requestEntryCount; i++) {
  ...
  private void prepareGroupOp(List<LogEntry> logEntries, long commitIndex) {
    ...
    logEntries.stream().filter(logEntry -> logEntry.getIndex() > commitIndex
      && logEntry.getOperation() instanceof RaftGroupOp).findFirst().ifPresent(logEntry -> {
        ...
        node.updateGroupMembers(logEntry.getIndex(), groupOp.getMembers(),
  ...
  private void revertPreparedGroupOp(List<LogEntry> logEntries) {
    ...
    logEntries.stream().filter(logEntry -> logEntry.getOperation() instanceof RaftGroupOp)
  ...
  private RaftMessage createAppendEntriesFailureResponse(int term, long queryRound,
    ...
    return modelFactory.createAppendEntriesFailureResponseBuilder().setGroupId(node.getGroupId())
      .setSender(localEndpoint()).setTerm(term)
      .setExpectedNextIndex(message.getPreviousLogIndex() + 1).setQuerySequenceNumber(queryRound)
      .setFlowControlSequenceNumber(sequenceNumber).build();
  }
}
