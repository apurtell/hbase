...
class Firewall {
   ...
   * <p>
   * After this call, Raft messages of the given type sent to the given Raft endpoint are silently
   * dropped. the target Raft endpoint to drop Raft messages of the given type the type of the Raft
   ...
   * <p>
   * If there were drop-message rules for the given target Raft endpoint, they are replaced with a
   * drop-all-messages rule. the target Raft endpoint to drop all Raft messages
   */
  ...
  /**
   * Deletes the drop-message rule for the given target Raft endpoint and the Raft message type. the
   * target Raft endpoint to remove the drop-message rule the type of the Raft messages
   */
  ...
  /**
   * Deletes all drop-message and drop-all-messages rules created for the given target Raft
   * endpoint. the target Raft endpoint to remove all rules
   */
  ...
  /**
   * Drops Raft messages of the given type to all Raft endpoints. the type of the Raft messages to
   * be dropped
   */
  ...
  /**
   * Deletes the drop-message rule for the given Raft message type to any Raft endpoint. the type of
   * the Raft message to delete the rule
   */
   ...
   * <p>
   * Only a single alter rule can be created for a given Raft endpoint and a new alter rule
   * overwrites the previous one. the target Raft endpoint to apply the alter function the alter
   * function to apply to Raft messages
   */
  ...
  /**
   * Deletes the alter-message rule for the given Raft endpoint. the target Raft endpoint to delete
   * the alter function
   */
  ...
  /**
   * Returns true if the given Raft message sent to the given Raft endpoint should be dropped. the
   * target Raft endpoint to check if the Raft message should be dropped the Raft message to check
  ...
  /**
   * Alters the given Raft message sent to the given Raft endpoint if there is an alter function for
   * the Raft message type. If there exists no alter function for the given Raft message type, then
   * the given Raft message is returned as it is. the target Raft endpoint to which the given Raft
   * message is sent the Raft message to alter if there exists an alter function for its type
    ...
    DropRule(RaftEndpoint endpoint, Class<? extends RaftMessage> type) {
      if (endpoint == null && type == null) {
        throw new IllegalArgumentException(
          "Cannot create a drop-all-messages to drop-all-endpoints rule!");
      }
