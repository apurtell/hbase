...
@Tag(SmallTests.TAG)
public class TestHigherTermStepDown extends TestBase {
  ...
  public void testAppendSuccessHigherTermStepsDown() {
  ...
  @Test
  @Timeout(value = 120, unit = TimeUnit.SECONDS)
  public void testInstallSnapshotHigherTermStepsDown() {
    group = LocalRaftGroup.newBuilder(3).setConfig(RaftTestUtils.TEST_RAFT_CONFIG)
      .enableNewTermOperation().start();
    RaftNodeImpl leader = group.waitUntilLeaderElected();
    leader.replicate(applyValue("x")).join();
  ...
  @Test
  @Timeout(value = 120, unit = TimeUnit.SECONDS)
  public void testVoteResponseHigherTermStepsDown() {
    group = LocalRaftGroup.newBuilder(3).setConfig(RaftTestUtils.TEST_RAFT_CONFIG)
      .enableNewTermOperation().start();
    RaftNodeImpl leader = group.waitUntilLeaderElected();
    leader.replicate(applyValue("x")).join();
